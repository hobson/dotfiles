...
# sudo yum install -y nmap nfs-utils nfs-common nfs nfs-client
...
#sudo yum install -y git hg git-hg nano meld anki xchat ghex gksudo python ipython python-devel python-yaml python-jinja2 gcc-c++ freetype-devel libpng-devel gnome-alternative-status-menu gnome-tweak-tool chrome pygtk2-devel 
...
#
# # image processing
# sudo yum install -y f-spot LibRaw perl-Image-ExifTool ufraw dcraw darktable dcraw ufraw-gimp gimp
#
...
# Django
#sudo yum install -y postgres postgresql postgresql-libs postgresql-devel psycopg
